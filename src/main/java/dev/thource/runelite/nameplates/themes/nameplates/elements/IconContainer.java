...
public class IconContainer extends Element {
    ...
    protected transient final Icon icon = new Icon();
    ...
    public IconContainer(String name, PositionProvider xPositionProvider, PositionProvider yPositionProvider, int iconSize, int padding, boolean isVertical, List<IconType> iconTypes) {
    ...
    public void draw(Nameplate nameplate, Graphics2D graphics, int x, int y, int plateWidth, int plateHeight) {
        var iconsToDraw = iconTypes.stream().filter(iconType -> Icon.shouldDraw(nameplate, iconType)).toArray(IconType[]::new);
