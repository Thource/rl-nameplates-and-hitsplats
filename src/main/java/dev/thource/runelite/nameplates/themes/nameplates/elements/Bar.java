...
public abstract class Bar extends Element {
    ...
    public Bar(String name, PositionProvider xPositionProvider, PositionProvider yPositionProvider, int width, int height, int borderSize, PositionProvider textXPositionProvider, PositionProvider textYPositionProvider, Color borderColor, Color backgroundColor, Color textColor, BarColorProvider barColorProvider) {
    ...
    public void draw(Nameplate nameplate, Graphics2D graphics, int plateX, int plateY, int plateWidth, int plateHeight) {
        ...
        Rect.draw(graphics, x + borderSize, y + borderSize, (int) (innerWidth * getProgress(nameplate)), innerHeight, barColorProvider.getColor(nameplate));

        Text.draw(graphics, x, y, textXPositionProvider, textYPositionProvider, getText(nameplate), textColor);
    }
}
