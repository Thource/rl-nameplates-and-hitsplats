...
public class Text extends Element {
    ...
    public Text(String name, PositionProvider xPositionProvider, PositionProvider yPositionProvider, Color color, NameplateTextProvider textProvider) {
    ...
    public void draw(Nameplate nameplate, Graphics2D graphics, int x, int y, int plateWidth, int plateHeight) {
        ...
        graphics.drawString(text, x + xPositionProvider.getValue() + textXOffset, y + yPositionProvider.getValue() + textYOffset);
    }

    public static void draw(Graphics2D graphics, int x, int y, PositionProvider xPositionProvider, PositionProvider yPositionProvider, String text, Color color) {
        graphics.setFont(FontManager.getRunescapeSmallFont());
        var fontMetrics = graphics.getFontMetrics();
        var textBounds = fontMetrics.getStringBounds(text, graphics);
        graphics.setColor(color);
        graphics.drawString(text, x + xPositionProvider.get(null, (int) textBounds.getWidth()), y + (int) textBounds.getHeight() + yPositionProvider.get(null, (int) textBounds.getHeight()));
    }
}
